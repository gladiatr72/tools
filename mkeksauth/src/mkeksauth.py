...
class EksAuth(object):
    ...
    def _users(self, users: Tuple[Tuple[str, str]]) -> None:
        # provided with an AWS IAM user name, add it to the stack

        rc = self.mc.resource("iam")

        assignments = [
            {"userarn": aws_user.arn, "username": aws_user.name, "groups": [k8s_group]}
            for aws_user in rc.users.all()
            for user, k8s_group in users
            if aws_user.name == user
        ]
...
def cli(ctx, cluster_name, context, group, user, role, verbose, output_file, apply):
    ...
    if not ctx.invoked_subcommand:
        if not (ctx.params["group"] or ctx.params["role"] or ctx.params["user"]):
            click.secho("at least one --role, --user or --group is required\n", fg="yellow")
            ready = False
